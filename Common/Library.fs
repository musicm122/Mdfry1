...
type AudioManager() =
    ...
    member this.initializeChannel busName = 
        ...
        | ex -> 
            let gEx = GodotSignalException (ex, $"connecting {channel.GetType().ToString()} signal 'finished' to {nameof(this.onStreamFinished)} failed")
            raise gEx
        
    member this.initializeChannels = 
        let initChannelOnMaster(_)=
            this.initializeChannel Bus
        [0..TotalChannels] 
