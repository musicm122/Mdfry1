...
public static class InputUtils
{
    ...
    {
        return Input.IsActionJustPressed(InputAction.ToggleFlashlight) ||
    ...
    {
        return Input.IsActionJustPressed(InputAction.Interact);
    }

    public static Vector2 GetTopDownMovementInput(float speed = 1f)
    {
        var velocity = Vector2.Zero;
        if (Input.IsActionPressed(InputAction.Right)) velocity.x += 1f;

        if (Input.IsActionPressed(InputAction.Left)) velocity.x -= 1f;

        if (Input.IsActionPressed(InputAction.Up)) velocity.y -= 1f;

        if (Input.IsActionPressed(InputAction.Down)) velocity.y += 1f;
    ...
    public static Vector2 GetCameraMovementInput(float speed = 1f)
    {
        ...
        if (Input.IsActionPressed(InputAction.CameraRight)) velocity.x += 1f;

        if (Input.IsActionPressed(InputAction.CameraLeft)) velocity.x -= 1f;

        if (Input.IsActionPressed(InputAction.CameraUp)) velocity.y -= 1f;

        if (Input.IsActionPressed(InputAction.CameraDown)) velocity.y += 1f;

        return velocity.Normalized() * speed;
    }

    public static bool IsCameraReset()
    {
        return Input.IsActionJustPressed(InputAction.CameraReset);
    }

    public static bool IsShooting()
    {
        return Input.IsActionJustPressed(InputAction.Shoot);
    }
}